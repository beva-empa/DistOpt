...
time_end = dt.datetime(2018, 1, 2, 0, 0, 0)
time_now = time_start
...
if __name__ == '__main__':
    ...
    def main_optimize(elec_storage, battery_depth, thermal_storage, CHP_Runtime, CHP_Downtime, capacities_conv,
        ...
        while cond:
            ...
            if cond and iter >= 20:
                flag = 0
                if 40 > iter:
                    for t in range(int(2 * num_opt_var / 3)):
                        if count[t] > 0:
                            flag = 1
                elif 60 > iter >= 40:
                    for t in range(int(num_opt_var / 3)):
                        if count[t] > 0:
                            flag = 1
                elif 60 <= iter:
                    flag = 1
    ...
    time_init = time_now
    elec_storage_in = elec_storage
    battery_depth_in = battery_depth
    thermal_storage_in = thermal_storage

    for j in range(2):

        p_avg_b1 = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        z_list_P_b1 = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        lagran_P_b1 = np.array([[[0.0] * int(num_hubs * (num_hubs - 1))] * int(num_hubs)] * num_opt_block)

        h_avg_b1 = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        z_list_H_b1 = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        lagran_H_b1 = np.array([[[0.0] * int(num_hubs * (num_hubs - 1))] * int(num_hubs)] * num_opt_block)
        ...
        #                 p_avg_b1[t][count] = 0.0
        #                 z_list_P_b1[t][count] = 0.0
        #                 h_avg_b1[t][count] = 0.0
        #                 z_list_H_b1[t][count] = 0.0
        #
        #             count += 1
            ...
            for rt in range(num_hubs):
                for st in range(num_hubs):
                    if st != rt:
                        if rt == hub:
                            for t in range(num_opt_block):
                                lagran_P_b1[t][hub][count] = -0.12
                                lagran_H_b1[t][hub][count] = -0.1
                        elif st == hub:
                            for t in range(num_opt_block):
                                lagran_P_b1[t][hub][count] = 0.12
                                lagran_H_b1[t][hub][count] = 0.1
                            ...
                            for t in range(num_opt_block):
                                lagran_P_b1[t][hub][count] = 0.0
                                lagran_H_b1[t][hub][count] = 0.0
                        count += 1
        ...
        (demand_power, demand_heat, power, heat, grid_in, grid_out, storage_heatin, storage_heatout, storage_elecin,
         ...
         z_list_H, lagran_H, itera) = small_block(elec_storage_in, battery_depth_in, thermal_storage_in, CHP_Runtime,
                                                  CHP_Downtime, capacities_conv, capacities_stor, tech_details,
                                                  storage_details, network_cap, list_techs, list_storage,
                                                  lagran_P_b1, z_list_P_b1, p_avg_b1, lagran_H_b1, z_list_H_b1,
                                                  h_avg_b1, demand_data, time_init)

        cnt = 0

        for t in range(j * num_opt_block, (j + 1) * num_opt_block):
            p_avg_in[t] = p_avg[cnt].tolist()
            z_list_P_in[t] = z_list_P[cnt].tolist()
            lagran_P_in[t] = lagran_P[cnt].tolist()

            h_avg_in[t] = h_avg[cnt].tolist()
            z_list_H_in[t] = z_list_H[cnt].tolist()
            lagran_H_in[t] = lagran_H[cnt].tolist()

            cnt += 1

        time_init += dt.timedelta(hours=12)

        elec_storage_in = storage_elec
        thermal_storage_in = storage_heat
