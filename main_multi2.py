...
if __name__ == '__main__':
    ...
    def main_optimize(elec_storage, battery_depth, thermal_storage, CHP_Runtime, CHP_Downtime, capacities_conv,
                      capacities_stor, tech_details, storage_details, network_cap, list_techs, list_storage, lagran,
                      z_list, p_avg, time_now):
        ...
        while cond:
            ...
            for hub in range(num_hubs):
                ...
                for item in list_storage[hub]:
                    ...
                    if item == 'heat_storage':
                        ...
                        constr += [(comp_list[it].P_BatteryTot[0] == Eff_Battery * elec_storage[hub] +
                                    Eff_BatteryCh * comp_list[it].P_BatteryCh[0] - (1 / Eff_BatteryDc) *
                                    comp_list[it].P_BatteryDc[0])]
                        ...
                        for t in range(num_opt_var):
                            ...
                            if t >= 1:
                                constr += [(comp_list[it].P_BatteryTot[t] == Eff_Battery *
                                            comp_list[it].P_BatteryTot[t - 1] + Eff_BatteryCh *
                                            comp_list[it].P_BatteryCh[t] - (1 / Eff_BatteryDc) *
                                            comp_list[it].P_BatteryDc[t])]
                        it = it + 1
                ...
                for idx in range(len(ex_list)):
                    ...
                    for t in range(num_opt_var):
                        cost += (trcost * tran_list[int(ex_list[idx])].P_strt[t] -
                                 lagran[t][hub][int(ex_list[idx])] * (tran_list[int(ex_list[idx])].P_strt[t] -
                                                                      z_list[t][int(ex_list[idx])]) +
                                 (delta * 0.5) * (tran_list[int(ex_list[idx])].P_strt[t] -
                                                  z_list[t][int(ex_list[idx])])**2)

                for idx in range(len(im_list)):
                    power_list.append(tran_list[int(im_list[idx])].P_strt * 0.95)
                    for t in range(num_opt_var):
                        cost += (lagran[t][hub][int(im_list[idx])] * (tran_list[int(im_list[idx])].P_strt[t] -
                                                                      z_list[t][int(im_list[idx])]) +
                                 (delta * 0.5) * (tran_list[int(im_list[idx])].P_strt[t] -
                                                  z_list[t][int(im_list[idx])])**2)
                ...
                for idx in range(len(ex_list)):
                    tr_cost += trcost * tran_list[int(ex_list[idx])].P_strt.value[0]#+ tran_ex[idx].Q_strt.value[0])
                ...
                for idx in range(len(tran_list)):
                    if idx in ex_list:
                        tr_pw.append(tran_list[idx].P_strt.value)
                    elif idx in im_list:
                        tr_pw.append(tran_list[idx].P_strt.value)
                    else:
                        tr_pw.append(empty)

                transfer_pw.append(tr_pw)

            p_avg_new = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_var)
            z_list_new = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_var)
            lagran_new = np.array([[[0.0] * int(num_hubs * (num_hubs - 1))] * int(num_hubs)] * num_opt_var)

            for id in range(num_hubs):
                for idx in range(int((num_hubs) * (num_hubs - 1))):
                    if check_array[id][idx] == 1:
                        for t in range(num_opt_var):
                            p_avg_new[t][idx] += 0.5 * transfer_pw[id][idx][t]
                            z_list_new[t][idx] += 0.5 * transfer_pw[id][idx][t] + (0.5/delta) * lagran[t][id][idx]

            for id in range(num_hubs):
                for idx in range(int(num_hubs * (num_hubs - 1))):
                    if check_array[id][idx] == 1:
                        for t in range(num_opt_var):
                            lagran_new[t][id][idx] = lagran[t][id][idx] + delta * (transfer_pw[id][idx][t] -
                                                                                   z_list_new[t][idx])

            prim_resid = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_var)
            dual_resid = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_var)
            dual_2norm = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_var)
            prim_2norm = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_var)

            for id in range(num_hubs):
                for idx in range(int(num_hubs * (num_hubs - 1))):
                    if check_array[id][idx] == 1:
                        for t in range(num_opt_var):
                            prim_resid[t][idx] += (transfer_pw[id][idx][t] - p_avg_new[t][idx])**2

            for idx in range(int(num_hubs * (num_hubs - 1))):
                for t in range(num_opt_var):
                    dual_resid[t][idx] = 2 * (delta**2) * (p_avg_new[t][idx] - p_avg[t][idx])**2

            cond = False

            for idx in range(int(num_hubs * (num_hubs - 1))):
                for t in range(num_opt_var):
                    dual_2norm[t][idx] = math.sqrt(dual_resid[t][idx])
                    prim_2norm[t][idx] = math.sqrt(prim_resid[t][idx])
                    if dual_2norm[t][idx] >= 0.05 or prim_2norm[t][idx] >= 0.05:
                        cond = True

            for idx in range(int(num_hubs * (num_hubs - 1))):
                for t in range(num_opt_var):
                    p_avg[t][idx] = p_avg_new[t][idx]
                    z_list[t][idx] = z_list_new[t][idx]

            for id in range(num_hubs):
                for idx in range(int(num_hubs * (num_hubs - 1))):
                    for t in range(num_opt_var):
                        lagran[t][id][idx] = lagran_new[t][id][idx]
        ...
        return demand_power, demand_heat, power, heat, grid_in, grid_out, storage_heatin, storage_heatout, \
               ...
               storage_heat, storage_elec, CHP_ontime, CHP_offtime, p_avg, z_list, lagran, iter
    ...
    # power_out = np.array([[0.00] * num_hubs] * num_opt_var)
    # heat_out = np.array([[0.00] * num_hubs] * num_opt_var)
    #
    # for hub in range(1, num_hubs + 1):
    #     (pow_out, he_out) = single_optimize(elec_storage, battery_depth, thermal_storage, CHP_Runtime, CHP_Downtime,
    #                                         capacities_conv, capacities_stor, tech_details, storage_details,
    #                                         list_techs, list_storage, demand_data, hub, time_now)
    #     for t in range(num_opt_var):
    #         power_out[t][hub-1] = pow_out[t]
    #         heat_out[t][hub-1] = he_out[t]

    count = 0
    p_avg_in = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_var)
    z_list_in = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_var)
    lagran_in = np.array([[[0.0] * int(num_hubs * (num_hubs - 1))] * int(num_hubs)] * num_opt_var)
        ...
        for rt in range(num_hubs):
            if st != rt:
                for t in range(num_opt_var):
                    ...
                    z_list_in[t][count] = 0.0
                    for hub in range(num_hubs):
                        # lagran[t][hub][count] = power_out[t][st]
                        lagran_in[t][hub][count] = 0.0
                count += 1
    ...
    while time_now <= time_end:
        ...
        (demand_power, demand_heat, power, heat, grid_in, grid_out, storage_heatin, storage_heatout, storage_elecin,
         ...
         CHP_ontime, CHP_offtime,
         p_avg, z_list, lagran, iter) = main_optimize(elec_storage, battery_depth, thermal_storage, CHP_Runtime,
                                                      CHP_Downtime, capacities_conv, capacities_stor, tech_details,
                                                      storage_details, network_cap, list_techs, list_storage,
                                                      lagran_in, z_list_in, p_avg_in, time_now)
        ...
        p_avg_final.append(p_avg[0])
        z_list_final.append(z_list[0])
        lagran_final.append(lagran[0])
        iter_final.append(iter)
        ...
        for t in range(num_opt_var - 1):
            p_avg_in[t] = p_avg[t + 1]
            z_list_in[t] = z_list[t + 1]
            lagran_in[t] = lagran[t + 1]

        p_avg_in[num_opt_var - 1] = p_avg[0]
        z_list_in[num_opt_var - 1] = z_list[0]
        lagran_in[num_opt_var - 1] = lagran[0]
    ...
    with xlsxwriter.Workbook('results_trcost1.xlsx') as workbook:
        ...
        saveHeat(workbook, demand_heat_final, heat_final, storage_heatin_final, storage_heatout_final, list_techs,
        ...
        savetrans(workbook, p_avg_final, z_list_final, lagran_final, date_info, time_info, num_hubs)
