...
from main_blockinit import *


def small_block(elec_storage, battery_depth, thermal_storage, CHP_Runtime, CHP_Downtime, capacities_conv,
                capacities_stor, tech_details, storage_details, network_cap, list_techs, list_storage, lagran_P,
                z_list_P, p_avg, lagran_H, z_list_H, h_avg, demand_data, time_now):
    ...
    while cond:
        ...
        demand_power = []
        demand_heat = []
        transfer_ht = np.array([[[0.0] * (num_hubs * (num_hubs - 1))] * num_hubs] * num_opt_block)
        transfer_pw = np.array([[[0.0] * (num_hubs * (num_hubs - 1))] * num_hubs] * num_opt_block)
        check_array = []
        ...
        for hub in range(num_hubs):
            ...
            for item in list_techs[hub]:
                ...
                if item == 'solar_PV':
                    ...
                    comp_list.append(PV(num_opt_block))
                    ...
                    comp_list.append(PVT(num_opt_block))
                    ...
                    for t in range(num_opt_block):
                        constr += [comp_list[it].P_PVT[t] >= 0, comp_list[it].Q_PVT[t] >= 0,
                                   comp_list[it].Out_PVT[t] <= (comp_list[it].b_PVT[t] * Eff_PVT *
                                                                comp_list[it].TempEff_PVT[t] * comp_list[it].I_PVT[
                                                                    t]),
                    ...
                    comp_list.append(mCHP(num_opt_block))
                    ...
                    comp_list.append(CHP(num_opt_block))
                    ...
                    comp_list.append(GSHP(num_opt_block))
                    ...
                    comp_list.append(GB(num_opt_block))
            ...
            for item in list_storage[hub]:
                ...
                if item == 'heat_storage':
                    ...
                    comp_list.append(Heat_Storage(num_opt_block))
                    ...
                    comp_list.append(Elec_Storage(num_opt_block))
            ...
            comp_list.append(Elec_Grid(num_opt_block))
            ...
            if len(power_list) < 35:
                lnp = len(power_list)
                while lnp <= 34:
                    power_list.append([0] * num_opt_block)
                    lnp += 1

            if len(heat_list) < 35:
                lnp = len(heat_list)
                while lnp <= 34:
                    heat_list.append([0] * num_opt_block)
                    lnp += 1
            ...
            problem.solve(solver=cp.MOSEK, verbose=True, save_file='opt_diagnosis.opf',
                          mosek_params={mosek.iparam.intpnt_solve_form: mosek.solveform.dual,
                                        mosek.dparam.optimizer_max_time: 300.0})
            ...
            for item in list_storage[hub]:
                ...
                if item == 'heat_storage':
                    st_heatin.append(-comp_list[it].Q_StorageCh.value[num_opt_block-1])
                    st_heatout.append(comp_list[it].Q_StorageDc.value[num_opt_block-1])
                    st_heat.append(comp_list[it].Q_StorageTot.value[num_opt_block-1])
                    it = it + 1

                if item == 'Battery':
                    st_elecin.append(-comp_list[it].P_BatteryCh.value[num_opt_block-1])
                    st_elecout.append(comp_list[it].P_BatteryDc.value[num_opt_block-1])
                    st_elec.append(comp_list[it].P_BatteryTot.value[num_opt_block-1])
        ...
        p_avg_new = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        z_P_new = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        lagran_P_new = np.array([[[0.0] * int(num_hubs * (num_hubs - 1))] * int(num_hubs)] * num_opt_block)

        h_avg_new = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        z_H_new = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        lagran_H_new = np.array([[[0.0] * int(num_hubs * (num_hubs - 1))] * int(num_hubs)] * num_opt_block)
        ...
        prim_P_resid = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        dual_P_resid = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)

        prim_H_resid = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        dual_H_resid = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        ...
        dual_P_2norm = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        prim_P_2norm = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)

        dual_H_2norm = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)
        prim_H_2norm = np.array([[0.0] * int(num_hubs * (num_hubs - 1))] * num_opt_block)

        cond = False
        count = [0] * num_opt_block
        for idx in range(int(num_hubs * (num_hubs - 1))):
            for t in range(num_opt_block):
        ...
        if cond and iter >= 20:
            flag = 0
            if 40 > iter:
                for t in range(int(2 * num_opt_block / 3)):
                    if count[t] > 0:
                        flag = 1
            elif 60 > iter >= 40:
                for t in range(int(num_opt_block / 3)):
                    if count[t] > 0:
                        flag = 1
            elif 60 <= iter:
                flag = 0
